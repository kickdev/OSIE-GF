...
int CIniFile::GetIntValue(string KeyName, string SectionName, string FileName)
{
	return atoi(CIniFile::GetValue(KeyName, SectionName, FileName).c_str());
}

int CIniFile::GetBoolValue(string KeyName, string SectionName, string FileName)
{
	return strcmp(CIniFile::GetValue(KeyName, SectionName, FileName).c_str(), "true") == 0 ? true : false;
}
...
string CIniFile::GetValueEx(string KeyName, string FileName)
{
	size_t nPos = 0;
	string sBuffer;
	...
	if(file.is_open())
	{
		...
		{
			nPos = sBuffer.find(KeyName.c_str());
			if(nPos != -1)
			{
				return sBuffer.substr(nPos+KeyName.size(), sBuffer.size()-nPos);
			}
		}
		file.close();
	}
